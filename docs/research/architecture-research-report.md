...
Local, non-production POCs are specified in [`docs/poc/README.md`](../poc/README.md). They started as standard-Node-only contract/payload checks written before the Astro workspace existed; as of Phase 2 checkpoint 2, all four originally deferred there (Content Collection validation, draft exclusion, real Pagefind indexing, analytics-script measurement) are complete against the real workspace - see that file for current status. They establish contract and payload boundaries, not provider SLAs.
...
3. Astro workspace, typed Content Collections, and schema validation built and passing (Phase 2 gate in `docs/roadmap.md`) - **done** as of Phase 2 checkpoint 1; see `docs/poc/README.md` for what's verified against it.
