...
4. `publication-gate` CI check wired into branch protection before any scheduled build is enabled — a `status: approved` frontmatter field alone must not be able to publish.
