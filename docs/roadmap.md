...
- Add build-time archive pagination with 10 live entries per page for `/articles` and `/brief`, overflow routes at `/articles/page/{n}` and `/brief/page/{n}`, and accessible no-JS links. Homepage latest-12, topic, series, and author listings remain separate; detail-page recommendations remain unimplemented. **Done**.
