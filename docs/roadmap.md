...
- Add build-time archive pagination with 10 live entries per page for `/articles` and `/brief`, overflow routes at `/articles/page/{n}` and `/brief/page/{n}`, and accessible no-JS links. Homepage latest-12, topic, series, and author listings remain separate. Add conservative build-time detail-page recommendations: explicit `related` article references (up to three) retain declared order when they resolve to approved live articles; article pages may show the first eligible next article in series order; both detail-page types fill remaining related-reading slots with deterministic shared-topic matches, excluding the current entry, duplicates, archived/draft/future/unapproved entries, and without client-side fetching or ranking. **Done**.
